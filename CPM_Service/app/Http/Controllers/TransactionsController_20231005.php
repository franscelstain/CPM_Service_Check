...
class TransactionsController extends AppController
{
    ...
    public function portfolio_balance(Request $request)
    {
        ...
        {
            $res    = [];
            $user   = in_array($this->auth_user()->usercategory_name, ['Investor', 'Sales']) ? $this->auth_user()->usercategory_name == 'Investor' ? 'b.investor_id' : 'b.sales_id' : '';
            $ymd    = !empty($request->date) ? ' '. $request->date : '';
            
            if ($this->auth_user()->usercategory_name == 'Sales')
                $no = !empty($request->month) ? $request->month : 1;
            else
		...
		if ($this->auth_user()->usercategory_name == 'Sales')
                {
                    $d1     = $i > 8 ? date('Y-'. ($i+1) .'-d') : date('Y-0'. ($i+1) .'-d');
                    $date   = Carbon::createFromFormat('Y-m-d', $d1)->format('Y-m-d') === $d1 ? $d1 : date('Y-m-t', strtotime(substr($d1, 0, -3)));
                }
                else
                {
                    $date = date('Y-m-d', strtotime('-'. $i .' days '. $ymd));
                }
                $asset  = $product = [];
                $c      = $g = $r = 0;
                //$date   = $this->auth_user()->usercategory_name == 'Sales' ? $i > 8 ? date('Y-'. ($i+1) .'-d') : date('Y-0'. ($i+1) .'-d') : date('Y-m-d', strtotime('-'. $i .' days '. $ymd));
                
		$data   = TransactionHistoryDay::join('u_investors as b', 't_trans_histories_days.investor_id', '=', 'b.investor_id')
                        ->join('m_products as c', 't_trans_histories_days.product_id', '=', 'c.product_id')
                        ->where([['history_date', $date], ['b.is_active', 'Yes'], ['c.is_active', 'Yes'], ['t_trans_histories_days.is_active', 'Yes']]);
                        
                if (empty($user))
                {
                    $res[$date] = floatval($data->sum('current_balance'));
                }
                else
                {
                    $data   = $data->selectRaw('LEFT(portfolio_id, 1) as portfolio, account_no, c.product_id, product_name, d.asset_class_id as id, d.asset_class_name, d.asset_class_color, SUM(current_balance) as balance')
                            ->leftJoin('m_asset_class as d', function($qry) { $qry->on('c.asset_class_id', '=', 'd.asset_class_id')->where('d.is_active', 'Yes'); })
                            ->where($user, $this->auth_user()->id)
                            ->groupBy(['portfolio', 'account_no', 'c.product_id', 'product_name'], 'id', 'd.asset_class_name', 'd.asset_class_color')
                            ->get();
                    foreach ($data as $dt)
                    {
                        switch ($dt->portfolio)
                        {
                            case 2  : $g += $dt->balance; break;
                            case 3  : $r += $dt->balance; break;
                            default : $c += $dt->balance;
                                if ($i == 0)
                                {
                                    $product[] = ['product_name' => $dt->product_name, 'balance_amount' => $dt->balance, 'account_no' => $dt->account_no];
                                    if (in_array($dt->id, array_keys($asset)))
                                    {
                                        $asset[$dt->id]['amount'] += $dt->balance;
                                    }
                                    else
                                    {
                                        $asset[$dt->id] = ['name' => $dt->asset_class_name, 'amount' => floatval($dt->balance), 'color' => $dt->asset_class_color];
                                    }
                                }
                        }
                    }
                    $res[$date] = ['current' => ['asset' => array_values($asset), 'balance' => floatval($c), 'product' => $product], 'goals' => floatval($g), 'retirement' => floatval($r)];
                }
            }
    ...
    public function sales_fee()
    {
        try
        {
            ...
            for ($i = 1; $i <= 12; $i++)
            {
                $date           = $i > 9 ? date('Y-m-d') : date('Y-0'.$i.'-d');
                $data           = TransactionFeeOutstanding::join('u_investors as b', 't_fee_outstanding.investor_id', '=', 'b.investor_id')
                                ->where([['t_fee_outstanding.is_active', 'Yes'], ['b.is_active', 'Yes'], ['b.sales_id', $auth->id], ['fee_date', $date]])
                                ->sum('fee_amount');
                $res[$date]     = floatval($data);
            }
            return $this->app_response('Sales Fee', $res);
        }
        catch (\Exception $e)
        {
            return $this->app_catch($e);
...
//return $this->app_response('cc', $api_feetax);
            if (!empty($api_feetax->original['message']))
            {
                if(!empty($api_feetax->original['message']->Result) && !empty($api_feetax->original['message']->Result->_TaxFeeListOut))
                {
