...
class InvestorTypesController extends AppController
{
    public $table = 'SA\Reference\KYC\InvestorType';
    ...
    public function ws_data(Request $request)
    {
        try
        {
            $insert = $update = 0;
            $data   = [];
            $grp    = Group::where([['group_name', 'InvestorType'], ['is_active', 'Yes']])->first();
            if (!empty($grp->group_id))
            {
                $api = $this->api_ws(['sn' => 'ReferenceGroupDetail', 'val' => [$grp->ext_code]])->original['data'];
                foreach ($api as $a)
                {                
                    $qry    = InvestorType::where([['ext_code', $a->code]])->first();
                    $id     = !empty($qry->investor_type_id) ? $qry->investor_type_id : null;
                    $request->request->add([
                        'investor_type_name'    => $a->name,
                        'ext_code'              => $a->code,
