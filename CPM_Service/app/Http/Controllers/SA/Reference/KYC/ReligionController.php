...
use App\Models\SA\Reference\KYC\Religion;
use Illuminate\Http\Request;

class ReligionController extends AppController
{
    public $table = 'SA\Reference\KYC\Religion';
    ...
    public function ws_data(Request $request)
    {
        try
        {
            $insert = $update = 0;
            $data   = [];
            $grp    = Group::where([['group_name', 'Agama'], ['is_active', 'Yes']])->first();
            if (!empty($grp->group_id))
            {
                $api    = $this->api_ws(['sn' => 'ReferenceGroupDetail', 'val' => [$grp->ext_code]])->original['data'];
                foreach ($api as $a)
                {                
                    $qry    = Religion::where([['ext_code', $a->code]])->first();
                    $id     = !empty($qry->religion_id) ? $qry->religion_id : null;
                    $request->request->add([
                        'religion_name' => $a->name,
                        'ext_code'      => $a->code,
