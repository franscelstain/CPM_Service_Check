    ...
    public function portfolio_balance(Request $request)
    {
        try
        {
            ...
            if ($this->auth_user()->usercategory_name == 'Sales')
            {
                ...
                $currentDate = date('Y-m-d'); // Tanggal hari ini, misal: '2024-06-15'
                $currentMonth = intval(date('m')); // Bulan saat ini, misal: 6
                $currentYear = intval(date('Y')); // Tahun saat ini, misal: 2024

                // Ambil nilai dari request->month
                $requestMonth = intval($request->input('month', $currentMonth)); // Default ke bulan aktif

                // Rentang waktu untuk bulan sebelumnya (Januari hingga bulan sebelum request atau bulan aktif)
                $startDate = sprintf('%04d-01-01', $currentYear); // Awal tahun
                $endMonth = min($requestMonth, $currentMonth - 1); // Ambil bulan terakhir sebelum bulan aktif
                $endDate = sprintf('%04d-%02d-%02d', $currentYear, $endMonth, date('t', strtotime(sprintf('%04d-%02d-01', $currentYear, $endMonth)))); // Akhir bulan terakhir

                // Query
                $dates = DB::table('t_trans_histories_days')
                        ->select(DB::raw("DISTINCT ON (date_trunc('month', history_date)) history_date"))
                        ->where(function ($query) use ($startDate, $endDate, $currentDate, $currentMonth, $requestMonth) {
                            if ($currentMonth > 1) {
                                // Jika bulan aktif bukan Januari, ambil rentang waktu dari Januari hingga bulan sebelum aktif
                                $query->whereBetween('history_date', [$startDate, $endDate]); // Tanggal dari Januari hingga bulan sebelum aktif
                                if ($requestMonth >= $currentMonth) {
                                    // Jika request->month mencakup bulan aktif, hanya ambil tanggal hari ini
                                    $query->orWhere('history_date', $currentDate);
                                }
                            } else {
                                $query->whereBetween('history_date', [$startDate, $currentDate]);
                            }
                        ...
                        ->orderByRaw("date_trunc('month', history_date), history_date DESC") // Urutkan berdasarkan bulan dan tanggal terbaru
                        ->pluck('history_date');
                ...
                //         ->orderByRaw("date_trunc('month', history_date), history_date DESC")
                //         ->pluck('history_date');
            }
            else
            {
