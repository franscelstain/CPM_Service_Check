...
use App\Models\Administrative\Notification\Category;
use App\Models\Administrative\Notification\Investor;
use App\Models\Administrative\Notification\InvestorInterval;
...
{
	public $table = 'Administrative\Notification\Investor';
 	...
 	public function index()
    {
        ...
        {
            $data   = [];
            $notif  = Investor::select('m_notification_investor.*', 'b.category_name')
            ...
            return $this->app_response('Notif Investor Setup', ['key' => 'id', 'list' => $notif]);
        }
        catch(\Exception $e)
        {
    ...
    public function category()
    {
        try
        {
            $data = Category::where([['assign_to', 'Investor'], ['is_active', 'Yes']])->get();
    ...
    public function interval($id)
    {
        try
        {
            $qry    = InvestorInterval::where([['investor_notif_id', $id], ['is_active', 'Yes']])->get();
    ...
    public function save(Request $request, $id = null)
    {
        try
        {
            ...
            if ($id)
            {
                ...
                InvestorInterval::where('investor_notif_id', $id)->update(['is_active' => 'No']);
                
                for ($i = 0; $i < count($reminder); $i++)
                {
                    $count  = $reminder[$i] == 'H' ? [] : [['count_reminder', $c_reminder[$i]]];
                    $intvl  = InvestorInterval::where(array_merge([['investor_notif_id', $id], ['reminder', $reminder[$i]]], $count))->first();
                    ...
                    $save   = empty($intvl->id) ? InvestorInterval::create($data) : InvestorInterval::where('id', $intvl->id)->update($data);
                }
            }
    ...
    //                 ->where([['h_notification_investor.is_active', 'Yes'], ['h_notification_investor.notif_send', false], ['b.is_active', 'Yes']])
    //                 ->where(function($qry) { $qry->where('h_notification_investor.notif_email', true)->orWhere('h_notification_investor.notif_mobile', true); })
    //                 ->get();
    ...
    //             {
    //                 NotificationInvestor::where('id', $dt->id)->update(['notif_send' => true]);
    //                 $res[] = $api['data'];
    //             }
    //             else
    //             {
