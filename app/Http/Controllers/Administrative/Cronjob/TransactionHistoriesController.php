...
class TransactionHistoriesController extends AppController
{
    ...
    private function save_history_current($inv_id)
    {
        ...
        $subQuery = DB::table('t_assets_outstanding as tao')
                ...
                ->where([
                    ['tao.investor_id', $inv_id],
                    ['tao.is_active', 'Yes'],
                    ['mp.is_active', 'Yes'],
                    ['tao.outstanding_date', $this->app_date()],
                ])
                ->groupBy('tao.investor_id', 'tao.product_id', 'tao.account_no');
        $asset  = DB::table('t_assets_outstanding as tao')
                ->joinSub($subQuery, 'latest', function($join) {
                    $join->on('tao.investor_id', '=', 'latest.investor_id');
                    $join->on('tao.product_id', '=', 'latest.product_id');
                    $join->on('tao.account_no', '=', 'latest.account_no');
                    $join->on('tao.data_date', '=', 'latest.latest_data_date');
                })
                ->join('m_products as mp', 'tao.product_id', '=', 'mp.product_id')
                ->join('m_asset_class as mac', function($join) { 
                    $join->on('mp.asset_class_id', '=', 'mac.asset_class_id')
                        ->where('mac.is_active', 'Yes');
                })
                ->join('m_asset_categories as mact', function($join) {
                    $join->on('mac.asset_category_id', '=', 'mact.asset_category_id')
                ...
                ->selectRaw('
                    ...
                    regular_payment, 
                    SUM(tao.outstanding_unit) as unit, 
                    SUM(tao.total_subscription) as total_sub, 
                    SUM(tao.total_unit) as total_unit, 
                    SUM(tao.balance_amount) as balance
                ')
                ->groupBy(['tao.account_no', 'mp.product_id', 'mact.diversification_account', 'tao.regular_payment'])
                ->get();
        foreach ($asset as $a)
        {
            $save   = true;
