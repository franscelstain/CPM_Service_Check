...
use App\Models\SA\Reference\KYC\Gender;
use Illuminate\Http\Request;

class GenderController extends AppController
{
    public $table = 'SA\Reference\KYC\Gender';
    ...
    public function ws_data(Request $request)
    {
        try
        {
            $insert = $update = 0;
            $data   = [];
            $grp    = Group::where([['group_name', 'Sex'], ['is_active', 'Yes']])->first();
            if (!empty($grp->group_id))
            {
                $api = $this->api_ws(['sn' => 'ReferenceGroupDetail', 'val' => [$grp->ext_code]])->original['data'];
                foreach ($api as $a)
                {
                    $qry    = Gender::where([['ext_code', $a->code]])->first();
                    $id     = !empty($qry->gender_id) ? $qry->gender_id : null;
                    $request->request->add([
                        'gender_name'   => $a->name,
                        'ext_code'      => $a->code,
