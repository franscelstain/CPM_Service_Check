        ...
        try {
            ...
            $data = TransactionHistory::select('g.reference_code', 't_trans_histories.*', 'b.fullname', 'b.photo_profile', 'b.cif', 'c.product_name', 'd.asset_class_name', 'e.reference_name as status_name', 'f.reference_name as trans_reference_name', 'f.reference_code as trans_reference_code', 'f.reference_color', 'g.reference_name as type_reference_name', 'h.issuer_logo', 'i.account_no as bank_account_no', 'i.account_name as bank_account_name')
                ...
                ->leftJoin('m_asset_class as d', function ($qry) {
                    return $qry->on('c.asset_class_id', '=', 'd.asset_class_id')->where('d.is_active', 'Yes');
                })
                ->leftJoin('m_trans_reference as e', function ($qry) {
                    return $qry->on('t_trans_histories.status_reference_id', '=', 'e.trans_reference_id')->where([['e.reference_type', 'Goals Status'], ['e.is_active', 'Yes']]);
                })
                ->leftJoin('m_trans_reference as f', function ($qry) {
                    return $qry->on('t_trans_histories.trans_reference_id', '=', 'f.trans_reference_id')->where([['f.reference_type', 'Transaction Status'], ['f.is_active', 'Yes']]);
                })
                ->leftJoin('m_trans_reference as g', function ($qry) {
                    return $qry->on('t_trans_histories.type_reference_id', '=', 'g.trans_reference_id')->where([['g.reference_type', 'Transaction Type'], ['g.is_active', 'Yes']]);
                })
                ->leftJoin('m_issuer as h', function ($qry) {
                    return $qry->on('c.issuer_id', '=', 'h.issuer_id')->where('h.is_active', 'Yes');
                })
                ->leftJoin('u_investors_accounts as i', function ($qry) {
                    return $qry->on('t_trans_histories.investor_account_id', '=', 'i.investor_account_id')->where('i.is_active', 'Yes');
                })
    ...
    public function report_invbalance(Request $request)
    {
        ...
        try {
            ...
            $data2 = LiabilityOutstanding::select("liabilities_name", "account_id", "outstanding_balance as amount")
                ->where([['investor_id', $inv_id], ['is_active', 'Yes'], ['outstanding_date', ">=", $cpm_date], ['outstanding_date', "<=", $cpm_date]])->get();

            $investor = DB::table('u_investors')
                ->where([['investor_id', $inv_id]])->first();
            ...
            return $this->app_response('Report', ['report1' => $data1, 'report2' => $data2, 'customer' => $investor]);
        } catch (\Exception $e) {
            return $this->app_catch($e);
        }
    }
            ...
            if (isset($inv->investor_id)) {
                ...
                if (!empty($email)) {
                    ...
                    foreach ($email as $m) {
                        $mail = [
                            'to' => $m,
                            'content' => 'Consolidated Statement',
